...
protected:
    ...
    fujiDeviceID_t _devnum;  ///< Device number (ID)
     ...
     */
    uint8_t bus_to_peripheral(uint8_t *buf, uint16_t len);
...
public:
     ...
     *
     * @param packet Referece to the packet data
     */
    void processCommand(FujiBusPacket &packet);
     ...
     * @param device_id Device ID (from fujiDeviceID_t)
     */
    void addDevice(RS232DeviceBase *pDevice, fujiDeviceID_t device_id);
     ...
     */
    RS232DeviceBase* deviceById(fujiDeviceID_t device_id);
     ...
     */
    void changeDeviceId(RS232DeviceBase *pDevice, fujiDeviceID_t device_id);
